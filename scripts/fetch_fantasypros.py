#!/usr/bin/env python3
"""Fetch FantasyPros superflex half-PPR consensus rankings (38 experts).

Writes data/rankings_fantasypros.json with one entry per player:
  {name, position, team, bye, fp_rank_overall, fp_rank_pos, fp_tier,
   fp_adp_avg, fp_rank_min, fp_rank_max, fp_expert_count}

The merger in fantasy_draft.rankings_overlay layers these onto the
players_2026.csv at load time. The endpoint is free (no auth) and pulls
from the public consensus URL FantasyPros embeds in its rankings pages.
"""
from __future__ import annotations
import json
import sys
import urllib.request
from pathlib import Path

ROOT = Path(__file__).resolve().parent.parent
OUT = ROOT / "data" / "rankings_fantasypros.json"

URL = (
    "https://partners.fantasypros.com/api/v1/consensus-rankings.php"
    "?sport=NFL&year=2026&week=0&position=OP&type=ROS&scoring=HALF&export=json"
)


def main() -> None:
    req = urllib.request.Request(URL, headers={"User-Agent": "Mozilla/5.0"})
    print(f"[FP] fetching {URL[:80]}...")
    with urllib.request.urlopen(req, timeout=30) as resp:
        payload = json.loads(resp.read().decode("utf-8"))
    n_experts = payload.get("total_experts", "?")
    last_updated = payload.get("last_updated", "?")
    players = payload.get("players", [])
    print(f"  Got {len(players)} players ({n_experts} experts, "
          f"updated {last_updated})")

    out = []
    for p in players:
        out.append({
            "name": p.get("player_name", "").strip(),
            "position": p.get("player_position_id", "").strip(),
            "team": p.get("player_team_id", "").strip(),
            "bye": p.get("player_bye_week"),
            "fp_rank_overall": p.get("rank_ecr"),
            "fp_rank_pos": p.get("pos_rank"),
            "fp_tier": p.get("tier"),
            "fp_adp_avg": p.get("rank_ave"),
            "fp_rank_min": p.get("rank_min"),
            "fp_rank_max": p.get("rank_max"),
            "fp_rank_std": p.get("rank_std"),
            "fp_expert_count": n_experts,
        })

    OUT.parent.mkdir(parents=True, exist_ok=True)
    payload_out = {
        "source": "FantasyPros partners API",
        "url": URL,
        "scoring": "half_ppr",
        "format": "superflex (OP)",
        "last_updated": last_updated,
        "total_experts": n_experts,
        "n_players": len(out),
        "players": out,
    }
    OUT.write_text(json.dumps(payload_out, indent=2), encoding="utf-8")
    print(f"  Wrote {OUT.relative_to(ROOT)}")


if __name__ == "__main__":
    main()
