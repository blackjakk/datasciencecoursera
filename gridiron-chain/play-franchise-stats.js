...
// per-player tallies. Returns { team:{tid:{epa,plays,epa_def,plays_def}},
// qb:Map(name → {epa,db}),  rec:Map(name → {epa,rec}),  rb:Map(name → {epa,att}),
// totals:{pPass,pRun,ePass,eRun,succP,succR} }.
//
...
function _mffComputeEPA() {
  const empty = { team: {}, qb: new Map(), rec: new Map(), rb: new Map(),
                  totals:{pPass:0, pRun:0, ePass:0, eRun:0, succP:0, succR:0} };
  ...
  let i = 0;
  while (i < log.length) {
    if (log[i] && log[i].__g) { curMeta = log[i]; i++; continue; }
    let end = i;
    ...
    if (game.length) {
      ...
      for (let j = 0; j < game.length; j++) {
        ...
        // Score that fires between c and next (or between c and end-of-game).
        let scored = null;
        ...
        if (scored) {
          ...
          epA = 0;   // end of half or end of game with no detected score
        }
        const epa = epA - epB;
        const offTid = curMeta ? teamByHomeAway(c.p, curMeta) : null;
        const defTid = curMeta ? teamByHomeAway(c.p === "home" ? "away" : "home", curMeta) : null;
        if (offTid != null) {
          const t = out.team[offTid] = out.team[offTid] || { epa:0, plays:0, epa_def:0, plays_def:0 };
          t.epa += epa; t.plays++;
        }
        if (defTid != null) {
          const t = out.team[defTid] = out.team[defTid] || { epa:0, plays:0, epa_def:0, plays_def:0 };
          t.epa_def += epa; t.plays_def++;
        }
        if (_MFF_EPA_PASS_KIND.has(c.k)) {
          out.totals.pPass++; out.totals.ePass += epa; if (epa > 0) out.totals.succP++;
          if (c.qb) { const q = bump(out.qb, c.qb, () => ({epa:0, db:0})); q.epa += epa; q.db++; }
          if (c.k === "complete" && c.rc) {
            const r = bump(out.rec, c.rc, () => ({epa:0, rec:0}));
            r.epa += epa; r.rec++;
          }
        } else {
          out.totals.pRun++; out.totals.eRun += epa; if (epa > 0) out.totals.succR++;
          if (c.ru) { const r = bump(out.rb, c.ru, () => ({epa:0, att:0})); r.epa += epa; r.att++; }
        }
      }
    }
    i = end;
...
// Team-level EPA (off + def per play). null only when there's truly no
// data (no offensive OR defensive plays yet). Either side may be null if
// that side hasn't accumulated plays.
...
function mffPlayerEPAChipsHtml(p) {
  ...
  if (r.kind === "qb" && r.db >= 10) {
    ...
    chips.push(chip("EPA " + fmtSignedSum(r.epa), tierFromEPA(perDb),
      `Total EPA on dropbacks`));
  ...
  return `<div style="margin-top:.5rem;padding-top:.4rem;border-top:1px dashed var(--border)">
    <div class="frn-card-title" style="margin-bottom:.25rem">EPA <span style="opacity:.6;font-weight:normal">(expected points added vs baked baseline)</span></div>
    <div style="display:flex;flex-wrap:wrap">${chips.join("")}</div>
  </div>`;
}

// Team EPA rows for the win-prob matchup compare block. Returns "" if no
// data yet (week 1 pre-game, etc.) — render falls back gracefully. Each
// side may be null (e.g. team played only home games this season) — show
// "—" for missing sides so the row still renders informatively.
